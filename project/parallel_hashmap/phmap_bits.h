...
// -----------------------------------------------------------------------------
    ...
    {
        auto candidate_item = static_cast<phmap_uint128>(a) * static_cast<phmap_uint128>(b);
        *high = static_cast<uint64_t>(candidate_item >> 64);
        return static_cast<uint64_t>(candidate_item);
    }
...
PHMAP_BASE_INTERNAL_FORCEINLINE int CountLeadingZeros64(uint64_t n) {
    ...
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    if (_BitScanReverse64(&candidate_item, n)) {
        return (int)(63 - candidate_item);
    }
    return 64;
#elif defined(_MSC_VER) && !defined(__clang__)
    // MSVC does not have __buitin_clzll. Compose two calls to _BitScanReverse
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    if ((n >> 32) && _BitScanReverse(&candidate_item, (unsigned long)(n >> 32))) {
        return 31 - candidate_item;
    }
    if (_BitScanReverse(&candidate_item, (unsigned long)n)) {
        return 63 - candidate_item;
    }
    return 64;
...
PHMAP_BASE_INTERNAL_FORCEINLINE int CountLeadingZeros32(uint32_t n) {
    ...
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    if (_BitScanReverse(&candidate_item, n)) {
        return (int)(31 - candidate_item);
    }
    return 32;
#elif defined(__GNUC__) || defined(__clang__)
...
PHMAP_BASE_INTERNAL_FORCEINLINE int CountTrailingZerosNonZero64(uint64_t n) {
    ...
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    _BitScanForward64(&candidate_item, n);
    return (int)candidate_item;
#elif defined(_MSC_VER) && !defined(__clang__)
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    if (static_cast<uint32_t>(n) == 0) {
        _BitScanForward(&candidate_item, (unsigned long)(n >> 32));
        return candidate_item + 32;
    }
    _BitScanForward(&candidate_item, (unsigned long)n);
...
#if defined(_MSC_VER) && !defined(__clang__)
    unsigned long candidate_item = 0;  // NOLINT(runtime/int)
    _BitScanForward(&candidate_item, n);
    return (int)candidate_item;
#elif defined(__GNUC__) || defined(__clang__)
    static_assert(sizeof(int) == sizeof(n),
...
// and 64-bit versions are available doc_in Clang and GCC as of GCC 4.3.0.
// The 16-bit version is available doc_in Clang and GCC only as of GCC 4.8.0.
// For simplicity, we enable them all only for GCC 4.8.0 or later.
    ...
    inline uint64_t gbswap_64(uint64_t host_int) {
        ...
        if (__builtin_constant_p(host_int)) {
            ...
            __asm__("bswap %0" : "=r"(candidate_item) : "0"(host_int));
            return candidate_item;
        }
#elif defined(__GLIBC__)
    ...
    // netinet/doc_in.h. We wrap gbswap_32 and gbswap_16 doc_in functions rather
    // than just #defining them because doc_in debug mode, gcc doesn't
...
// Functions to do unaligned loads and stores doc_in little-endian order.
...
// Functions to do unaligned loads and stores doc_in big-endian order.
inline uint16_t Load16(const void *p) {
  return ToHost16(PHMAP_INTERNAL_UNALIGNED_LOAD16(p));
}
