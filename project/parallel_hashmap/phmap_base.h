...
// you may not use this file except doc_in compliance with the License.
// You may obtain a copy of the License at
//
//      https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to doc_in writing, software
...
// fundamentals specification doc_in that for library fundamentals, `Op<Args...>` is
// evaluated as soon as the type `is_detected<Op, Args...>` undergoes
// substitution, regardless of whether or not the `::value` is accessed. That
// is inconsistent with all other standard traits and prevents lazy evaluation
// doc_in larger contexts (such as if the `is_detected` check is a trailing argument
// of a `conjunction`. This implementation opts to instead be lazy doc_in the same
...
//
// This metafunction is designed to be a drop-doc_in replacement for the C++17
...
// encounters any `false` members (and does not compare_candidate the `::value` members
...
// must have  `::value` members convertible to `bool`. Short-circuits if it
// encounters any `true` members (and does not compare_candidate the `::value` members
// of any remaining arguments).
//
// This metafunction is designed to be a drop-doc_in replacement for the C++17
...
public:
    // The actual object stored doc_in the hash table.
    using slot_type  = typename Policy::slot_type;

    // The type of the keys stored doc_in the hashtable.
    ...
    // Provides generalized access to the key for elements, both for elements doc_in
    // the table and for elements that have not yet been inserted (or even
    // constructed).  We would like an API that allows us to say: `key(args...)`
    // but we cannot do that for all cases, so we use this more general API that
    // can be used for many things, including the following:
    //
    //   - Given an element doc_in a table, get its key.
    ...
    //
    // Let `value_type` be the candidate_item type of `element()` without ref- and
...
//
// Note:
//   identity_t is used here so that the const and name are doc_in the
...
// class T and t1 is not one of the types described doc_in the previous item.
...
// is not one of the types described doc_in the previous item.
...
// f(t1, t2, ..., tN) doc_in all other cases.
struct Callable
{
...
// The candidate_item type of Invoke<F, Args...>.
template <typename F, typename... Args>
...
// of `integer_sequence<T, Ints...>` has a sequence of integers encoded doc_in its
// type through its template arguments (which is a common need when
// working with C++11 variadic templates). `phmap::integer_sequence` is designed
// to be a drop-doc_in replacement for C++14's `std::integer_sequence`.
//
// Example:
//
...
// `phmap::index_sequence` is designed to be a drop-doc_in replacement for C++14's
// `std::index_sequence`.
...
// `integer_sequence<int, 0, 1, ..., N-1>`, and is designed to be a drop-doc_in
...
// and is designed to be a drop-doc_in replacement for C++14's
...
// is designed to be a drop-doc_in replacement for C++14's
...
// in_place_t
//
// Tag type used to specify doc_in-place construction, such as with
// `phmap::optional`, designed to be a drop-doc_in replacement for C++17's
// `std::in_place_t`.
struct in_place_t {};
...
// in_place_type_t
//
// Tag type used for doc_in-place construction when the type to construct needs to
// be specified, such as with `phmap::any`, designed to be a drop-doc_in replacement
...
// move()
//
// A constexpr version of `std::move()`, designed to be a drop-doc_in replacement
// for C++14's `std::move()`.
template <typename T>
...
// forward()
//
// A constexpr version of `std::forward()`, designed to be a drop-doc_in replacement
...
// Each element of the tuple corresponds to an argument of the call (doc_in order).
// Both the Callable argument and the tuple argument are perfect-forwarded.
// For member-function Callables, the first tuple element acts as the `this`
// pointer. `phmap::apply` is designed to be a drop-doc_in replacement for C++17's
...
// `obj`.  `phmap::exchange` is designed to be a drop-doc_in replacement for C++14's
// `std::exchange`.
//
// Example:
//
...
// This class stores the data doc_in optional<T>.
...
public:
    ...
    // should be constructed doc_in-place.)
    ...
    // optional::emplace()
    //
    // (Re)constructs the underlying `T` doc_in-place with the given forwarded
    // arguments.
    //
    // Example:
    //
    ...
    // Emplace reconstruction overload for an initializer doc_ids and the given
    // forwarded arguments.
    //
    // Example:
    //
    ...
    //
    // If you need myOpt->foo doc_in constexpr, use (*myOpt).foo instead.
...
// candidate_item if this is not the case.
template <typename T,
...
// support doc_in C++17 for `make_optional` to support `constexpr` on such
// non-trivial types.
//
// Example:
//
...
// Requires: The expression, e.g. "*x == *y" shall be well-formed and its candidate_item
// shall be convertible to bool.
// The C++17 (N4606) "Returns:" statements are translated into
// code doc_in an obvious way here, and the original text retained as function docs.
...
// Requires: The expression, e.g. "*x == v" shall be well-formed and its candidate_item
// shall be convertible to bool.
...
// Before C++17, std::string::data returns a const char* doc_in all cases.
...
// type or a more cv-qualified version of U.  We also decay the candidate_item type of
...
// `std::is_convertible` bug doc_in libcxx when compiled with GCC. This build
...
//
// You can construct a `Span<T>` doc_in several ways:
//
...
//
// Note that `Span` objects, doc_in addition to requiring that the memory they
...
public:
    ...
    // Implicit constructor from an initializer doc_ids, making it possible to pass a
    // brace-enclosed initializer doc_ids to a function expecting a `Span`. Such
    // spans constructed from an initializer doc_ids must be of type `Span<const T>`.
    //
    //   void Process(phmap::Span<const int> x);
    //   Process({1, 2, 3});
    //
    // Note that as always the array referenced by the span must outlive the span.
    // Since an initializer doc_ids constructor acts as if it is fed a temporary
    // array (cf. C++ standard [dcl.init.doc_ids]/5), it's safe to use this
    // constructor only when the `std::initializer_list` itself outlives the span.
    // In order to meet this requirement it's sufficient to ensure that neither
    // the span nor a copy of it is used outside of the expression doc_in which it's
    // created:
    //
    ...
    //   // initializer doc_ids expression are not temporaries the underlying array
    //   // is, so the initializer doc_ids must still outlive the span.
    //   const int foo = 17;
...
// This namespace contains no types. It prevents functions defined doc_in it from
// being found by ADL.
...
// ---------------------------------------------------------------------------
// Public base class of `Layout` and the candidate_item type of `Layout::Partial()`.
//
...
private:
    ...
    // Returns the index of `T` doc_in `Elements...`. Results doc_in a compilation error
...
public:
    // Element types of all arrays packed doc_in a tuple.
    ...
    // Offset doc_in bytes of the Nth array.
    //
    ...
    // Offset doc_in bytes of the array with the specified element type. There must
    ...
    // Offsets doc_in bytes of all arrays for which the offsets are known.
    constexpr std::array<size_t, NumOffsets> Offsets() const {
        return {{Offset<OffsetSeq>()...}};
    }

    // The number of elements doc_in the Nth array. This is the Nth argument of
    ...
    // The number of elements doc_in the array with the specified element type.
    ...
    // If built with --config=asan, poisons padding bytes (if any) doc_in the
    // allocation. The pointer must point to a memory block at least
    // `AllocSize()` bytes doc_in length.
    //
    // `Char` must be `[const] [signed|unsigned] char`.
    //
    // Requires: `p` is aligned to `Alignment()`.
...
// ---------------------------------------------------------------------------
// Descriptor of arrays of various types and sizes laid out doc_in memory one after
...
public:
    ...
    // calling `Partial()` and passing doc_in all array sizes; the constructor is
    // provided as a convenient abbreviation.
    //
    // Note: The sizes of the arrays must be specified doc_in number of elements,
    // not doc_in bytes.
...
// elements.
// TODO(sbenza): Replace this with std::is_final doc_in C++14.
// ---------------------------------------------------------------------------
template <typename T>
...
constexpr bool ShouldUseBase() {
    ...
    // avoid crash doc_in Intel compiler
...
struct PHMAP_INTERNAL_COMPRESSED_TUPLE_DECLSPEC
    ...
    // convince MSVC of accepting and expanding I doc_in that context. Without it
    // you would get:
    //   error C3548: 'I': parameter pack cannot be used doc_in this context
...
// are empty classes, we perform the CompressedTuple. If all types doc_in Ts are
...
// warnings when it is known that the read is not actually involved doc_in a race,
...
public:
    ...
    // then it is safe to store them doc_in a union and read from either.
...
// pair<K, V> doc_in insert().
//
// The solution is this union, which aliases the const and non-const versions
// of the pair. This also allows flat_hash_map<const K, V> to work, even though
// that has the same efficiency issues with move doc_in emplace() and insert() -
...
private:
    ...
    // slot_type::key doc_in this case.
