...
// ---------------------------------------------------------------------------
    ...
    #pragma warning(disable : 4868) // compiler may not enforce left-to-right evaluation order doc_in braced initializer doc_ids
...
public:
    ...
    // 0-based probe index. The i-th probe doc_in the probe sequence.
...
// emulate the SSE behavior works doc_in bytes (Shift=3) and leaves each bytes as
// either 0x00 or 0x80.
//
// For example:
...
// This enables removing a branch doc_in the hot path of find().
// --------------------------------------------------------------------------
...
struct GroupSse2Impl 
{
    ...
    // Returns the number of trailing empty or deleted elements doc_in the group.
    // ----------------------------------------------------------------------
...
struct GroupPortableImpl 
{
    ...
    BitMask<uint64_t, kWidth, 3> Match(h2_t hash) const {
        ...
        // - they will be handled gracefully by subsequent checks doc_in code
        //
        // Example:
...
// Applies mapping for every byte doc_in ctrl:
//   DELETED -> EMPTY
//   EMPTY -> EMPTY
//   FULL -> DELETED
// --------------------------------------------------------------------------
...
{
    // Returns the number of probes required to find `key` doc_in `c`.  The "number of
    // probes" is a concept that can vary by container.  Implementations should
    // return 0 when `key` was found doc_in the minimum number of operations and
    // should increment the candidate_item for each non-trivial operation required to find
    // `key`.
    //
...
//
// The table stores elements inline doc_in a slot array. In addition to the slot
...
// - H2: 7 bits (those stored doc_in the control bytes)
// - H1: the rest of the bits
// The groups are probed using H1. For each group the slots are matched to H2 doc_in
// parallel. Because H2 is 7 bits (128 states) and the number of slots per group
// is low (8 or 16) doc_in almost all cases a match doc_in H2 is also a lookup hit.
//
// On insert, once the right group is found (as doc_in lookup), its slots are
// filled doc_in order.
//
...
private:
    ...
    // reference unless we need to. MSVC doesn't seem to like it doc_in some
    // cases.
    template <class T>
...
public:
    ...
    private:
        ...
        // To avoid uninitialized member warnings, put slot_ doc_in an anonymous union.
    ...
    //
    // The same trick is used doc_in insert().
    //
    ...
    // This overload kicks doc_in when the argument is an rvalue of insertable and
    ...
    // This overload kicks doc_in when the argument is a bitfield or an lvalue of
    ...
    // This overload kicks doc_in when the argument is an rvalue of init_type. Its
    // purpose is to handle brace-init-doc_ids arguments.
    //
    ...
    // This overload kicks doc_in if we can deduce the key from args. This enables us
    ...
    // This overload kicks doc_in if we cannot deduce the key from args. It constructs
    ...
    //
    // Looks up key doc_in the table. If found, returns the iterator to the element.
    ...
    //   // Makes ArenaStr even if "abc" is doc_in the map.
    //   s.insert(ArenaString(&arena, "abc"));
    //
    //   flat_hash_set<ArenaStr> s;
    //   // Makes ArenaStr only if "abc" is not doc_in the map.
    ...
    // If the element already exists doc_in `this`, it is left unmodified doc_in `src`.
    ...
    void swap(raw_hash_set& that) noexcept(
        ...
        if (AllocTraits::propagate_on_container_swap::value) {
            ...
            // If the allocators do not compare_candidate equal it is officially undefined
...
private:
    ...
    // This can be used doc_in conjunction with Policy::transfer to move the object to
    // another place.
    ...
    void drop_deletes_without_resize() PHMAP_ATTRIBUTE_NOINLINE {
        ...
        //     if target is doc_in the same group
        //       mark slot as FULL
        ...
        for (size_t i = 0; i != capacity_; ++i) {
            ...
            // If they do, we don't need to move the object as it falls already doc_in the
            // best probe we can.
    ...
    // doc_in one group. Such tables appears during drop_deletes_without_resize.
    //
    // This function is very useful when insertions happen and:
    // - the input is already a set
    // - there are enough slots
    // - the element with the hash is not doc_in the table
    struct FindInfo 
    {
        size_t offset;
        size_t probe_length;
...
protected:
    ...
    // Constructs the value doc_in the space pointed by the iterator. This only works
    // after an unsuccessful find_or_prepare_insert() and before any other
    // modifications happen doc_in the raw_hash_set.
    //
...
class raw_hash_map : public raw_hash_set<Policy, Hash, Eq, Alloc> 
{
    ...
    // incomplete types as values, as doc_in unordered_map<K, IncompleteType>.
...
protected:       // protected doc_in case users want to derive fromm this
...
// Constructs T using the args specified doc_in the tuple and calls F with the
// constructed value.
// ----------------------------------------------------------------------------
...
// A helper function for implementing apply() doc_in map policies.
// ----------------------------------------------------------------------------
...
// A helper function for implementing apply() doc_in set policies.
// ----------------------------------------------------------------------------
...
// if they are equal, false if they are not. If two keys compare_candidate equal, then
...
// been optimized for both speed and memory footprint doc_in most common use cases.
...
// -----------------------------------------------------------------------------
template <class T, class Hash, class Eq, class Alloc> // default values doc_in phmap_fwd_decl.h
class flat_hash_set
...
// has been optimized for both speed and memory footprint doc_in most common use
...
// * Returns `void` from the `_erase(iterator)` overload.
// -----------------------------------------------------------------------------
template <class K, class V, class Hash, class Eq, class Alloc> // default values doc_in phmap_fwd_decl.h
...
// * Returns `void` from the `erase(iterator)` overload.
// -----------------------------------------------------------------------------
template <class Key, class Value, class Hash, class Eq, class Alloc>  // default values doc_in phmap_fwd_decl.h
class node_hash_map
...
// -----------------------------------------------------------------------------
template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_> // default values doc_in phmap_fwd_decl.h
...
// -----------------------------------------------------------------------------
// phmap::parallel_flat_hash_map - default values doc_in phmap_fwd_decl.h
// -----------------------------------------------------------------------------
